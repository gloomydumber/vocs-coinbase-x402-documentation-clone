...
x402는 HTTP의 요청(Request)-응답(Response) 방식을 통한 간단한 프로그래밍 방식의 결제를 지원합니다.
...
5. **서버가 결제를 검증** - 리소스 서버는 다음 중 하나를 통해 결제 payload를 검증합니다.
   - 로컬(자체 검증을 실행하는 경우)
   - [**Facilitator**](https://docs.cdp.coinbase.com/x402/core-concepts/facilitator)를 통해서 (권장)
6. **Facilitator 검증** - Facilitator를 이용하는 경우, Facilitator는 지불을 Scheme 및 네트워크 요구 사항에 맞춰 검증하고 검증 Response를 반환합니다.
7. **서버가 요청을 처리** - 결제가 유효하면 서버는 원래 요청을 처리합니다. 유효하지 않으면 402 응답을 다시 반환합니다.
8. **결제 정산** - 서버는 다음 중 하나를 통해 블록체인 결제를 시작합니다.
   - 블록체인에 직접 제출
   - Facilitator의 `/settle` 엔드포인트를 통해서 제출
9. **Facilitator가 블록체인에 제출** - Facilitator는 결제 네트워크를 기반으로 블록체인에 거래를 브로드캐스트하고 확인을 기다립니다.
10. **결제 확인** - 체인상에서 확인되면, Facilitator는 결제 실행 응답을 반환합니다.
...
[**네트워크 및 Facilitator**](https://docs.cdp.coinbase.com/x402/network-support) - 사용 가능한 네트워크 및 Facilitator 옵션
...
- **블록체인 독립적** - Facilitator를 통해 여러 네트워크 지원
